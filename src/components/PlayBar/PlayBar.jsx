...
const Playbar = (props) => {
 const { isPlaying, currentObj, onNextTrack, onPrevTrack, isShuffle, onSetVolume, onScrub, toggleRepeat, toggleShuffle, togglePlay, isRepeat, progress, duration, volume, getFavoriteStateById, setTrackFromOrToFav } = props;
    ...
    <div className={s.playbar_content + " content_wrapper"}>
     <div className={s.current}>
      ...
      {currentObj ? (
       <>
        ...
        <button className={s.cr_fav} onClick={setTrackFromOrToFavCallback}>
         {" "}
     ...
     <div className={s.volume}>
      <div className={s.volume_content}>
       <span>{volume === 100 ? <IoVolumeHighOutline /> : volume >= 50 ? <IoVolumeMediumOutline /> : volume > 0 ? <IoVolumeLowOutline /> : <IoVolumeOffOutline />}</span>
...
export const FullCover = withClickOutside(({ currentObj, refE, setShow, isShow }) => {
 return (
  <div ref={refE}>
